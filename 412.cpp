...
int main (void) {
	...
	while (true) {
		...
		for (i = 0; i < n; ++i) {
			scanf("%d", &tmpI);
			if (tmpI % 2 == 0) listT[lenT++] = tmpI;
			else if (isPrime(tmpI)) listP[lenP++] = tmpI;
			else listO[lenO++] = tmpI;
		}

		cnt = 0;
		for (i = 0; i < lenO; ++i) {
			for (j = 0; j < lenT; ++j) if (gcd(listO[i], listT[j]) < 2) cnt ++;
			for (j = i + 1; j < lenO; ++j) if (gcd(listO[i], listO[j]) < 2) cnt ++;
		}

		for (i = 0; i < lenP; ++i) {
			if (listP[i] == 1) cnt += lenT + lenO;
			else {
				for (j = 0; j < lenT; ++j) if (listT[j] % listP[i] != 0) cnt ++;
				for (j = 0; j < lenO; ++j) if (listO[j] % listP[i] != 0) cnt ++;
			}
			for (j = i + 1; j < lenP; ++j) if (listP[i] != listP[j]) cnt ++;
		}
