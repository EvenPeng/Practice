...
int main (void) {
	...
	for (i = 0; i < 10; ++i) {
		check[i] = true;
		v[i][0] = i * 10000;
		for (j = 1; j < 5; ++j) v[i][j] = v[i][j - 1] / 10;
	}

	len = 0;
	for (i = 0; i < 10; ++i) {
			check[i] = false;
		for (j = 0; j <= i / 2; ++j) if (check[j]) {
			check[j] = false;
	...
	seg[list[0].v] = 1;
	for (i = 1; i < len; ++i) {
		if (list[i].v == list[i - 1].v) seg[list[i].v] ++;
		else {
			head[list[i].v] = i;
			seg[list[i].v] = 1;
		}
	...
	while (true) {
		...
		if (n < 2 || 79 < n || seg[n] == 0) printf("There are no solutions for %d.\n", n);
		else for (i = 0; i < seg[n]; ++i)
