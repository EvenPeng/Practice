...
int next (int y) {
	...
	x = y - 1;
	for (i = 1; i <= n; ++i)
	for (j = 1; j <= n; ++j) list[y].tran[i][j] = 0;
...
int main (void) {
	...
	while (scanf("%d", &n) != EOF) {
		for (i = 1; i <= n; ++i) for (j = 1; j <= n; ++j)
			if (i ^ j) scanf("%lf", &list[0].tran[i][j]);
			else list[0].tran[i][j] = 1.0;
		...
		if (!x) printf("no arbitrage sequence exists\n");
		else {
			seq[i + 1] = x;
			j = i;
			for (; i > 0; --i) seq[i] = list[i].pre[x][seq[i + 1]];
			printf("%d", x);
			for (i = 1; i <= j; ++i) printf(" %d", seq[i]);
			printf(" %d\n", x);
		}
	}
