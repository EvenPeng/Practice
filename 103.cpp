...
int comp(int x, int y)
{
    ...
    if (list[x][d] < list[y][d])
        return -1;
    else if (list[x][d] > list[y][d])
        return 1;

    for (i = 0; i < d; ++i)
        if (list[x][i] < list[y][i])
            return -1;
        else if (list[x][i] > list[y][i])
            return 1;
...
void sortH(int head, int len)
{
    ...
    while (true) {
        while (comp(label[head + i], pivot) < 0)
            ++i;
        while (comp(label[head + j], pivot) > 0)
            --j;
        if (i >= j) break;
...
bool check(int x, int y)
{
    ...
    for (i = 0; i < d; ++i)
        if (list[label[x]][i] >= list[label[y]][i]) return false;
