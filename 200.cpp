...
int main (void) {
	...
	while (scanf("%s", pA) != EOF) {
		if (pA[0] != '#') {
			lenA = strlen(pA);
			if (lenB > 0) {
				for (i = 0; pA[i] == pB[i]; ++i) check[map[pA[i]]] = true;
				if (i < lenA && i < lenB) {
			...
			len = 0;
			for (i = 0; i < 26; ++i) if (check[i]) list[len++] = i;
			while (len > 0) {
				for (i = 0; i < len; ++i) {
					flag = true;
					for (j = 0; flag && j < 26; ++j) flag &= !table[j][list[i]];
					if (flag) {
						printf("%c", 'A' + list[i]);
						for (j = 0; j < 26; ++j) table[list[i]][j] = false;
						tmpC = list[len - 1];
						list[len - 1] = list[i];
						list[i] = tmpC;
						len --;
