...
int comp (int x, int y) {
	if (count[x] - count[y] == 0) return x - y;
...
void sortD (int head, int len) {
	...
	while (true) {
		while (comp(order[i], pivot) < 0) ++i;
		while (comp(order[j], pivot) > 0) --j;
		if (i >= j) break;
...
int main (void) {
	...
	while (times-- > 0) {
		...
		for (i = 0; i < num; ++i) {
			for (j = 0; j < 4; ++j) accu[j] = 0;
			for (j = 0; j < len; j += sublen) {
				label = table[seq[i][j] - 'A'];
				sublen = 1;
				while (j + sublen < len) {
					if (seq[i][j] == seq[i][j + sublen]) sublen ++;
					else break;
				}
				accu[label] += sublen;
				for (k = label + 1; k < 4; ++k) count[i] += accu[k] * sublen;
			}
		}
