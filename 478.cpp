...
inline bool inR (fig *f, double x, double y) {
	if (x <= f->arr[0] || x >= f->arr[2] || y <= f->arr[1] || y >= f->arr[3]) return false;
	else return true;
}

inline bool inC (fig *f, double x, double y) {
	if (pow(f->arr[0] - x, 2) + pow(f->arr[1] - y, 2) >= pow(f->arr[2], 2)) return false;
	else return true;
}

inline bool inT (fig *f, double x, double y) {
	double a = -((x * f->arr[3] - y * f->arr[2]) - (f->arr[0] * f->arr[3] - f->arr[1] * f->arr[2])) / (f->arr[2] * f->arr[5] - f->arr[3] * f->arr[4]);
	double b = ((x * f->arr[5] - y * f->arr[4]) - (f->arr[0] * f->arr[5] - f->arr[1] * f->arr[4])) / (f->arr[2] * f->arr[5] - f->arr[3] * f->arr[4]);
...
int main (void) {
	...
	while (true) {
		...
		} else if (list[lenL].type[0] == 'c') scanf("%lf %lf %lf", list[lenL].arr, list[lenL].arr + 1, list[lenL].arr + 2);
		else if (list[lenL].type[0] == 't') {
			scanf("%lf %lf %lf %lf %lf %lf", list[lenL].arr, list[lenL].arr + 1, list[lenL].arr + 2, list[lenL].arr + 3, list[lenL].arr + 4, list[lenL].arr + 5);
